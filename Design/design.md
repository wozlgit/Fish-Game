...
+ Something allowing whales to be a fast and skilled-based form of transportation
+ Whales have to also function as an obstacle

Whale nests are randomly generated next to obstacles, just like plantation
Each whale has 3 states: HUNGRY, TIRED, SLEEPING
Hungry => Rotate slowly towards nearest powerup, all the while going in the current direction
Tired => First rotate VERY slowly towards Own Whale Nest, then do a REALLY fast surge to it
Sleeping => No movement and rotation for a fixed period of time

Hungry => Tired, when collides with powerup
Tired => Sleeping, when collides with target whale nest
Sleeping => Hungry, after sleeping time

- Players can "ride" the current generated by whales, moving at the same velocity in the same direction as the whale
(implement via the knockback system).
- Make this only work while the whale is surging to it's nest?
  + More clean movement
  - Getting stuck on a whale that is moving really slowly is annoying (and feels "laggy"?)
- The very slow rotation at the start of TIRED state serves as an indication to the player that the whale is about to surge
- The whale could slow down upon getting close to it's nest, so the player could use their own charge ability to get out of the whale's current, to avoid colliding with obstacles

? Whale momentum (+immersion)
> When a whale is spawned, it starts at the HUNGRY state, and it takes quite some time for it to reach a powerup,
and therefore transition to the TIRED state. After that it takes yet more time to finish rotating and to do the surge. After all that time, the player will already be way ahead the whale, and thus wont see or get to experience it's surge.
  ? Generate terrain (and thus also whales) some distance ahead of the player
...
Whale behaviour (whale nests) DONE
